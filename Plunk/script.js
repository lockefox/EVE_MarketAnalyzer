...
function crestChart() {
    ...
    var dim = {
        ...
        margin: { top: 20, right: 75, bottom: 30, left: 75 },
        ohlc: { height: 305 },
        indicator: { height: 65, padding: 10 }
    };
    ...
    function my(selection) {
      selection.each(function(d, i) {
        ...
        d3.select(this).append("button").text("Reset chart " + d.itemId + "!").on("click", reset);
        d3.select(this).append("br");
        d3.select(this).append("hr");   
        plotExample(d);
    
        function plotExample(ex) {
          
          ex.dataGetter(ex.dataUrl, function(error, data) {
              var accessor = candlestick.accessor(),
                  indicatorPreRoll = 33;  // Don't show where indicators don't have data
      
              data = ex.mapTechan(data, ex.parseDate)
                       .sort(function(a, b) { 
...
//              svg.select("g.close.annotation").datum([data[data.length-1]]).call(closeAnnotation);
    ...
    my.width = function(value) {
      if (!arguments.length) return dim.width;
      dim.width = value;
      return my;
    };

    my.height = function(value) {
      if (!arguments.length) return dim.height;
      dim.height = value;
      return my;
    };
    
    my.aspect_h = function(value) {
      if (!arguments.length) return dim.width / dim.height;
      dim.height = dim.width / value;
      return my;
    }
    
    my.aspect_w = function(value) {
      if (!arguments.length) return dim.width / dim.height;
      dim.width = dim.height * value;
      return my;
    }
    
    return my;
}
// "http://public-crest.eveonline.com/market/10000002/types/29668/history/";
function crestItem(itemId, regionId) {
  return {
    itemId: itemId,
    dataGetter: d3.json,
    dataUrl: "http://public-crest.eveonline.com/market/" + regionId + "/types/" + itemId + "/history/",
    mapTechan: function(data, parseDate) {
      return data.items.map(function(d) {
        return {
          date: d3.time.format("%Y-%m-%dT%X").parse(d.date),
          open: +d.avgPrice,
          high: +d.highPrice,
          low: +d.lowPrice,
          close: +d.avgPrice,
